...
**Related, corrected 2026-07-24:** `setSlashAllowance` is a raw set (not increase/decrease), so it carries the classic ERC-20 approve race. An earlier version of this line called the impact low "because already-locked obligations are unaffected". That reasoning was backwards, the in-flight obligation is exactly what keeps the grant alive. `release` restores capacity unconditionally (`AgentBond.sol:181`), so an enforcer holding one Active obligation can defeat a `setSlashAllowance(e, 0)` revocation by releasing and re-locking, indefinitely. The exposure **cap still holds** (nothing above the grant can be taken, so this is not theft above the cap), but "the agent can revoke at any time" is not true of the deployed contract. **Fixed and deployed 2026-07-31** on [`fix/agentbond-allowance-epoch`](https://github.com/Mnorbert87/bondwire/tree/fix/agentbond-allowance-epoch): grant generations, so a release only restores capacity under the current generation, plus race-free `increaseSlashAllowance` / `decreaseSlashAllowance`. 43/43 green there against 32/32 on the previous source. The sentence above describes the **previous** deployment; on the live AgentBond (`0x4383…702c`) a revoke is final. See THREAT_MODEL §1.
