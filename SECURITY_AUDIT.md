...
**Related, corrected 2026-07-24:** `setSlashAllowance` is a raw set (not increase/decrease), so it carries the classic ERC-20 approve race. An earlier version of this line called the impact low "because already-locked obligations are unaffected". That reasoning was backwards, the in-flight obligation is exactly what keeps the grant alive. `release` restores capacity unconditionally (`AgentBond.sol:181`), so an enforcer holding one Active obligation can defeat a `setSlashAllowance(e, 0)` revocation by releasing and re-locking, indefinitely. The exposure **cap still holds** (nothing above the grant can be taken, so this is not theft above the cap), but "the agent can revoke at any time" is not true of the deployed contract. Fix implemented and tested, not deployed, on [`fix/agentbond-allowance-epoch`](https://github.com/Mnorbert87/bondwire/tree/fix/agentbond-allowance-epoch): grant generations, so a release only restores capacity under the current generation, plus race-free `increaseSlashAllowance` / `decreaseSlashAllowance`. 43/43 green there against 32/32 on the deployed source. Not merged for the same reason as the CommitStakeV2 fix, the live AgentBond is source-verified and any source edit breaks the recompile-to-bytecode match. See THREAT_MODEL §1.
